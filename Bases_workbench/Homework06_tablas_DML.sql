DROP DATABASE Henry06;
CREATE DATABASE Henry06;
...
INSERT INTO instructor (Cedula_Identidad, nombre, apellido, Fecha_Nacimiento, Fecha_Incorporacion) 
...
INSERT INTO alumnos (IdAlumno,Cedula_Identidad,nombre,apellido,Fecha_nacimiento,Fecha_ingreso,Cohorte)
...
-- 2)
select * from cohorte where IdCohorte=1245;
...
-- 5)
SELECT IdAlumno,nombre, apellido, fecha_ingreso, cohorte
FROM alumnos
WHERE Cohorte = 1243;
...
SELECT *
FROM instructor
WHERE idInstructor IN (SELECT DISTINCT Instructor FROM cohorte WHERE Carrera = 1 ORDER BY Instructor);
...
SELECT * 
FROM alumnos
WHERE cohorte = 1235 AND Fecha_Ingreso LIKE "2019%";

-- 9)
SELECT alumnos.nombre, alumnos.apellido, alumnos.fecha_Nacimiento, carrera.nombre AS "Carrera"
FROM alumnos
INNER JOIN cohorte
ON alumnos.cohorte=cohorte.idCohorte
INNER JOIN carrera
ON cohorte.carrera = carrera.idCarrera;

/*a) IdCohorte desde alumno a cohorte, y IdCarrera de cohorte a Carrera. Los datos salen del ON

 b) relacion 1:M porque las cohorte tienen mas de 1 alumno

 c) WHERE carrera.nombre LIKE "Full Stack Developer" 
    WHERE carrera.nombre NOT LIKE "Data Science"

 d) WHERE carrera.nombre = "Full Stack Developer" 
    WHERE carrera.nombre != "Data Science"        En ambos casos, la manera correcta de buscar es la mas precisa, en este caso LIKE e = porwque si hay mas de dos carreras, con el NOT LIKE o el != me va a traer algunas carreras que tampoco quiero. Filtrar por valores que me van a arrojar un TRUE.

*/
-- e)

SELECT alumnos.nombre, alumnos.apellido, alumnos.fecha_Nacimiento, carrera.nombre AS 'Carrera', cohorte.IdCohorte
FROM alumnos
INNER JOIN cohorte
ON alumnos.cohorte=cohorte.idCohorte
INNER JOIN carrera
ON cohorte.carrera = carrera.idCarrera
WHERE carrera.IdCarrera = 1; -- IdCarrera 1 es Full Stack Developer
...
SELECT count(nombre)
FROM carrera;
-- En Henry hay 2 carreras

-- 2)
SELECT count(IdAlumno)
FROM alumnos;
-- En Henry hay 180 alumnos en total

-- 3)
SELECT COUNT(IdAlumno), cohorte
FROM alumnos
GROUP BY Cohorte;
-- En Henry hay 20 alumnos por cohorte

-- 4)
SELECT CONCAT(nombre, ' ', apellido) AS nombre_completo, Fecha_Ingreso
FROM alumnos
ORDER BY fecha_ingreso DESC;
-- Listado de alumnos ordenados por los ultimos alumnos que ingresaron, cocatenando nombre y apellido
...
LIMIT 1;
-- Nombre del primer alumno que ingresó a Henry
...
ORDER BY fecha_ingreso DESC
LIMIT 1;
-- El nombre del ultimo alumno que ingresó a Henry es Jason Harmon

-- 8)
SELECT YEAR(fecha_ingreso) AS año, COUNT(IdAlumno) AS cantidad_alumnos
FROM alumnos
GROUP BY YEAR(fecha_ingreso);
-- Cantidad de alumnos que ingresaron a Henry por año.

-- 9)
SELECT YEAR(fecha_ingreso) AS año, WEEKOFYEAR(fecha_ingreso) AS semana, COUNT(IdAlumno) AS cantidad_alumnos
FROM alumnos
GROUP BY año, semana
ORDER BY año;
-- Cantidad de alumnos que ingresaron por semana a henry.
...
-- años en los que ingresaron mas de 20 alumnos

-- 11)
SELECT TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) AS edad, nombre, apellido, Fecha_Nacimiento
FROM instructor;

SELECT DISTINCT concat(nombre, ' ', apellido) AS nombre_apellido, timestampdiff(YEAR, fecha_Nacimiento, curdate()) AS años,
date_add(fecha_Nacimiento,interval timestampdiff(Year,fecha_Nacimiento, curdate()) year) as verificacion, fecha_nacimiento
FROM instructor
ORDER BY 1;

-- 12)
SELECT concat(nombre, ' ', apellido) AS nombre_apellido, timestampdiff(YEAR, fecha_Nacimiento, curdate()) AS años,
...
SELECT idCohorte, avg(timestampdiff(YEAR, fecha_Nacimiento, curdate())) as edad_promedio
FROM alumno
GROUP BY idCohorte;
...
and fecha_Nacimiento < '1950-01-01';
